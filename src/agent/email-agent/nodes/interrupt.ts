...
export async function interruptNode(state: EmailAgentState): Promise<EmailAgentUpdate> {
  if (!state.email) {
...
- **Ignore**: Ignoring the email will end the conversation, and the email will not be sent.`;
  ...
  if (typeof res.args !== "object" || !res.args || !("subject" in res.args) || !("body" in res.args) || !("to" in res.args)) {
    throw new Error("If response type is edit, args must be an object with 'subject', 'body', and 'to' fields.")
  }
